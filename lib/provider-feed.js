...
function buildProviderFeed(result, options = {}) {
  ...
  for (const [sourceName, source] of Object.entries(sources)) {
    ...
    for (const match of matches) {
      ...
      const event = applyEventStateOverride(buildEvent(enrichedMatch, sourceName), marketStateStore);
      events.push(event);
      const eventMarkets = applyMarketStateOverrides(
        applyManualOverrides(buildMarkets(enrichedMatch, event.eventId), manualOddsStore),
        marketStateStore,
        event
      );
...
function buildEvent(match, sourceName) {
  ...
  return {
    ...
    coverage: {
      timeline: true,
      bookingEvents: true,
      derivedMarkets: Boolean(match.lambdas),
      manualOdds: true,
    },
    analytics: match.lambdas || null,
  };
}

function buildMarkets(match, eventId) {
  const markets = [];
...
function propagateRelatedMarketOverrides(markets) {
  const modelOverride = deriveEventModelOverride(markets);
  if (modelOverride) {
    return applyEventModelOverride(markets, modelOverride);
  }
  ...
  const probabilities = extractSelectionProbabilities(matchWinner, ["home", "draw", "away"]);
  if (!probabilities) {
    return markets;
  }
